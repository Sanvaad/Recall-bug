...
library LibGateway {
    ...
    function getBottomUpCheckpoint(uint256 epoch)
        internal
        view
    ...
    function commitParentFinality(ParentFinality calldata finality)
        internal
        returns (ParentFinality memory lastFinality)
    {
        GatewayActorStorage storage s = LibGatewayActorStorage.appStorage();
    ...
    function applyMsg(SubnetID memory arrivingFrom, IpcEnvelope memory crossMsg, bool expectTopDownOnly) internal {
        ...
        if (crossMsg.to.subnetId.isEmpty()) {
            sendReceipt(
                ...
                abi.encodeWithSelector(InvalidXnetMessage.selector, InvalidXnetMessageReason.DstSubnet)
            );
            return;
        }
        ...
        if (applyType == IPCMsgType.BottomUp) {
            ...
            if (subnet.appliedBottomUpNonce != crossMsg.localNonce) {
                sendReceipt(
                    crossMsg,
                    OutcomeType.SystemErr,
                    abi.encodeWithSelector(InvalidXnetMessage.selector, InvalidXnetMessageReason.Nonce)
                );
                return;
            }
        ...
        if (!crossMsg.to.subnetId.equals(s.networkName)) {
            ...
            if (!valid) {
                sendReceipt(
                    crossMsg, OutcomeType.SystemErr, abi.encodeWithSelector(InvalidXnetMessage.selector, reason)
                );
                return;
            }
    ...
    function executeCrossMsg(IpcEnvelope memory crossMsg, Asset memory supplySource)
        internal
    ...
    function checkCrossMessage(IpcEnvelope memory envelope)
        internal
        view
        returns (bool valid, InvalidXnetMessageReason reason, IPCMsgType applyType)
    {
