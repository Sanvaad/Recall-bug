    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _handleIpcCall(IpcEnvelope memory envelope, CallMsg memory callMsg)
        internal
        virtual
    ...
    function _handleIpcResult(IpcEnvelope storage original, IpcEnvelope memory result, ResultMsg memory resultMsg)
        internal
        virtual;

    /// @notice Method the implementation of this contract can invoke to perform an IPC call.
    function performIpcCall(IPCAddress memory to, CallMsg memory callMsg, uint256 value)
        internal
        nonReentrant
        returns (IpcEnvelope memory envelope)
    {
